...
module.exports.onStart = async function ({ api, event, args, usersData }) {
  ...
  if (!dipto) {
    return api.sendMessage(
      `❤️🫰 এই ভাবে ট্রাই করুন🫰❤️\n ${global.GoatBot.config.prefix}fbcover v1/v2/v3/v4/v5 - name - title - address - email - phone - color (default = white)`,
      event.threadID,
      event.messageID,
    );
  } else {
    ...
    api.sendMessage(
      `💚🫶 আপনার কভার তৌরী হচ্ছে প্লিজ ওয়েট🫶💚`,
      event.threadID,
      (err, info) =>
        setTimeout(() => {
