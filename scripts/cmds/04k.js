...
module.exports.onStart = async ({ api, event, args }) => {
  ...
  try {
    ...
    if (!event.messageReply || !event.messageReply.attachments || !event.messageReply.attachments[0]) {
      return api.sendMessage("𝙿𝚕𝚎𝚊𝚜𝚎 𝚛𝚎𝚙𝚕𝚢 𝚢𝚘𝚞𝚛 𝚙𝚑𝚘𝚝𝚘", event.threadID, event.messageID);
    }


    const B4D9L = event.messageReply?.attachments[0]?.url;


    const apiUrl = `https://mostakim.onrender.com/remini?input=${encodeURIComponent(B4D9L)}`;
    ...
    api.sendMessage({
      body: "𝚢𝚘𝚞𝚛 𝚙𝚒𝚌𝚝𝚞𝚛𝚎 𝚒𝚜 𝚏𝚞𝚕𝚕 𝙷𝙳 𝙳𝚘𝚗𝚎",
