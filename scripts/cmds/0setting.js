...
module.exports.onStart = async function({ api, args, event, Users,onReply,permssion, Threads }) {
...
module.exports.onReply = async function({
  args, event, Users,Threads, api, onReply, permssion
}) {
  const { threadID, messageID, senderID } = event;
  switch (onReply.type) {
    case "choosee": {
      switch (event.body) {
        case "1": {
             const permission = ["100049220893428"];
	...
	return api.sendMessage(`《Restarted successfully》`, threadID, () => process.exit(1));
         ...
         case "2": {
           ...
           const listAdmin = global.GoatBot.config.adminBot[0];
    if (senderID != listAdmin) return api.sendMessage("done -_-", threadID, messageID);
          delete require.cache[require.resolve(global.client.GoatBot.configPath)];
global.GoatBot.config = require(global.client.GoatBot.configPath);
...
var inbox = await api.getThreadList(100, null, ['INBOX']);
  ...
  for (var groupInfo of list) {
    ...
    return api.sendMessage(`Updated your data ${lengthGroup} box`, threadID)
        ...
        case "4": {
          if (event.senderID != "100049220893428") return api.sendMessage(`The age of the cock`, event.threadID, event.messageID)
...
api.logout()
 ...
 return api.sendMessage(`Is already ${(data["antiout"] == true) ? "bật" : "tắt"} Successful antiout!`, event.threadID);
    ...
    var phut = moment.tz("Asia/Ho_Chi_Minh").format("mm");
    var giay = moment.tz("Asia/Ho_Chi_Minh").format("ss");
    const namebot = config.nickNameBot
