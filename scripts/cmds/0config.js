...
onStart: async function({ event, commandName, message , api}) {
    message.reply({body:"╔════•|𝐁𝐀𝐃𝐀𝐋•𝐑𝐁-𝐁𝐎𝐓|•════╗\n\n[01] Edit bot bio"
 ...
 + "\n````````````````````````````````"}, (err, info) => {
  ...
  onReply: async function({ api, message, event, Reply, args }) {
  	/*==========LET=======*/
         let { type, author, commandName } = Reply;
...
if (Array.isArray(appState)) {
  ...
  console.error("appState is not an array");
}
    
         

       /*====START=======*/
  if (author != senderID) return message.reply("You dont have enough permission to use this command.");
     
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      message.reply(`Please reply to this message with ${["01", "1"].includes(args[0]) ? "bio" : "nickname"} you want to change to bot or 'delete' if you want to delete ${["01", "1"].includes(args[0]) ? "bio" : "nickname"} present`, (err, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      return message.reply(`Bot message waiting list:\n\n${msg}`);
    }
    else if (["04", "4"].includes(args[0])) {
      const messagePending = await api.getThreadList(500, null, ["unread"]);
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | Message: ${b.snippet}\n`, "") || "There are no messages yet";
      return message.reply(`Bot unread message list:\n\n${msg}`);
    }
    else if (["05", "5"].includes(args[0])) {
      const messagePending = await api.getThreadList(500, null, ["OTHER"]);
      const msg = messagePending.reduce((a, b) => a += `» ${b.name} | ${b.threadID} | Message: ${b.snippet}\n`, "") || "There are no messages yet";
      return message.reply(`Bot spam message list:\n\n${msg}`);
    }
    else if (["06", "6"].includes(args[0])) {
      message.reply(`Reply to this message with a photo or a link of the image you want to change to the bot avatar`, (err, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      if (!args[1] || !["on", "off"].includes(args[1])) return message.reply('Please select on or off');
      const form = {
        av: botID,
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (err, data) => {
        ...
        else message.reply(`» Is already ${args[1] == 'on' ? 'bật' : 'tắt'} successful bot avatar shield`);
      });
    }
    else if (["08", "8"].includes(args[0])) {
      return message.reply(`Reply to this message with the id of the person you want to block, you can enter multiple ids separated by a space or a newline`, (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      return message.reply(`Reply to this message with the id of the person you want to unblock, can enter multiple ids separated by space or newline`, (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      return message.reply(`Reply to this message with the content you want to create a post`, (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      return message.reply(`Respond to this message with the post id you want to delete, you can enter multiple ids separated by a space or a newline`, (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      return message.reply(`Reply to this message with the postID you want to comment on (post ${args[0] == "12" ? "by user" : "on group"}), can enter multiple ids separated by space or newline`, (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      message.reply(`Reply to this message with the desired post id ${args[0]  == "13" ? "release emotions" : args[0] == "14" ? "send friend invitations" : args[0] == "15" ? "accept friend request" : args[0] == "16" ? "decline friend request" : args[0] == "17" ? "delete friends" : "send Message"}, can enter multiple ids separated by space or newline`, (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      message.reply('Reply to this message with the code you want to create a note', (e, info) => {
        global.GoatBot.onReply.set(info.messageID, {
          commandName,
          messageID: info.messageID,
          author: senderID,
      ...
      api.logout((e) => {
        if (e) return message.reply('An error occurred, please try again later');
    ...
    api.changeBio(bio, false, (err) => {
      if (err) return message.reply("An error occurred, please try again later");
      else return message.reply(`Is already ${!bio ? "delete bot's profile successfully" : `change bot profile to: ${bio}`}`);
    });
  }
    ...
    if (nickname) {
      ...
      if (!res.includes('href="/profile/edit/info/nicknames/?entid=')) return message.reply('Bot của bạn hiện tại chưa đặt tên biệt danh nào');
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      if (e) return message.reply(`𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 please try again later`);
      else if (JSON.parse(i).errors) message.reply(`𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 Please try again later: ${JSON.parse(i).errors[0].summary}, ${JSON.parse(i).errors[0].description}`);
      else message.reply(`Is already ${!nickname ? "Delete the bot's nickname successfully" : `rename bot's nickname to: ${nickname}`}`);
    });
  }
    ...
    else return message.reply(`Please enter a valid image link or reply to the message with an image you want to set as an avatar for the bot`, (err, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    try {
      ...
      if (uploadImageToFb.error) return message.reply("𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 Please try again later: " + uploadImageToFb.error.errorDescription);
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
        if (e) message.reply(`𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 please try again later`);
        else if (JSON.parse(i.slice(0, i.indexOf('\n') + 1)).errors) reply(`𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 Please try again later: ${JSON.parse(i).errors[0].description}`);
        else message.reply(`Changed avatar for bot successfully`);
      });
    }
    catch(err) {
      message.reply(`𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 please try again later`);
    }
  }
  
  
  else if (type == 'blockUser') {
    if (!body) return message.reply("Please enter the uid of the people you want to block on messenger, you can enter multiple ids separated by space or newline", (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    message.reply(`» Successfully blocked ${success.length} users on messenger${failed.length > 0 ? `\n» Block failure ${failed.length} user, id: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'unBlockUser') {
    if (!body) return message.reply("Please enter uid of the people you want to unblock on messenger, you can enter multiple ids separated by space or newline", (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    message.reply(`» Unblocked successfully ${success.length} users on messenger${failed.length > 0 ? `\n» Unblock failure ${failed.length} user, id: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'createPost') {
    if (!body) return message.reply("Please enter the content you want to create the article", (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    api.httpPost('https://www.facebook.com/api/graphql/', form, (e, i) => {
      if (e || JSON.parse(i).errors) return message.reply(`Tạo bài viết thất bại, vui lòng thử lại sau`);
    ...
    if (!body) return message.reply('Please enter the id of the post you want to comment on', (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
        type: "choiceIdCommentPost",
        isGroup: Reply.isGroup
      });
    })
    message.reply("Reply to this message with the content you want to comment on the post", (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    const { postIDs, isGroup } = Reply;
    
    if (!body) return message.reply('Please enter the content you want to comment on the post', (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
        type: "commentPost",
        postIDs: Reply.postIDs,
        isGroup: Reply.isGroup
      });
    });
    const success = [];
    ...
    message.reply(`» Successfully commented ${success.length} posts${failed.length > 0 ? `\n» Comment failed ${failed.length} posts, postID: ${failed.join(" ")}` : ""}`);
  }
    ...
    for (const postID of postIDs) {
  		...
  		try {
           ...
           headers
  		  ...
  		  message.reply("𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 the post id does not exist or you are not the owner of this article");
  		}
      ...
      const data = "fb_dtsg=" + encodeURIComponent(fb_dtsg) +"&jazoest=" + encodeURIComponent(jazoest) + "&action_key=DELETE&submit=G%E1%BB%ADi";
        ...
        const dt = await axios({
  			...
  			if (dt.data.includes("Sorry, an badol has occurred")) throw new badol();
  			success.push(postID);
  		}
  		catch(err) {
  			failed.push(postID);
    ...
    message.reply(`» Deleted successfully ${success.length} posts${failed.length > 0 ? `\n»Delete failed ${failed.length} posts, postID: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'choiceIdReactionPost') {
    if (!body) return message.reply(`Please enter the post id you want to react to`, (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    message.reply(`Enter the emotion you want to react to ${listID.length} posts (unlike/like/love/heart/haha/wow/sad/angry)`, (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    const success = [];
    const failed = [];
    const postIDs = Reply.listID;
    const feeling = body.toLowerCase();
    if (!'unlike/like/love/heart/haha/wow/sad/angry'.split('/').includes(feeling)) return message.reply('Please choose one of the following emotions unlike/like/love/heart/haha/wow/sad/angry', (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    message.reply(`» Released emotions ${feeling} give ${success.length} successful post${failed.length > 0 ? `» Reaction failed ${failed.length} posts, postID: ${failed.join(" ")}` : ''}`);
  }
    ...
    message.reply(`» Friend request has been sent successfully to ${success.length} id${failed.length > 0 ? `\n» Send a friend request to ${failed.length} id failure: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'choiceIdSendMessage') {
    const listID = body.replace(/\s+/g, " ").split(" ");
    message.reply(`Enter the text of the message you want to send ${listID.length} user`, (e, info) => {
      global.GoatBot.onReply.set(info.messageID, {
        commandName,
        messageID: info.messageID,
        author: senderID,
    ...
    message.reply(`» Deleted successfully ${success.length} friend${failed.length > 0 ? `\n» Delete failed ${failed.length} friend:\n${failed.join("\n")}` : ""}`);
  }
  
  
  else if (type == 'sendMessage') {
    const listID = Reply.listID;
    const success = [];
    const failed = [];
    for (const uid of listID) {
      try {
    ...
    message.reply(`» Message sent successfully to ${success.length} user${failed.length > 0 ? `\n» Send a message to ${failed.length} user failed: ${failed.join(" ")}` : ""}`);
  }
    ...
    message.reply(`» Is already ${type == 'acceptFriendRequest' ? 'accept' : 'erase'} successful friend request of ${success.length} id${failed.length > 0 ? `\n» Fail with ${failed.length} id: ${failed.join(" ")}` : ""}`);
  }
    ...
    .then(response => {
      ...
      message.reply(`Create a successful note, link: ${'https://buildtool.dev/code-viewer.php?' + href}`)
    })
    .catch(err => {
      message.reply('𝐁𝟒𝐃𝟗𝐋-𝐁𝟗𝐓___//𝟎𝟎𝟕 please try again later');
    })
